...
const ReservationConfirmationEmail = (props: Props) => {
  const {
    customerName = 'Client',
    confirmationId = '--------',
    vehicleName = '',
  ...
  return (
    <Html lang="fr" dir="ltr">
      <Head />
      <Preview>Confirmation de votre réservation {confirmationId} — {SITE_NAME}</Preview>
      <Body style={main}>
        <Container style={container}>
          <Section style={header}>
            <Heading style={h1}>{SITE_NAME}</Heading>
            <Text style={subtitle}>CONFIRMATION DE RÉSERVATION</Text>
          </Section>
          ...
          <Text style={text}>
            Votre réservation <strong>N° {confirmationId}</strong> a été enregistrée avec succès. Voici le récapitulatif :
          </Text>

          <Section style={section}>
            <Text style={sectionTitle}>DÉTAILS DE LA RÉSERVATION</Text>
            <Text style={row}>Véhicule : <strong>{vehicleName}</strong></Text>
            <Text style={row}>Du : <strong>{pickupDate}</strong></Text>
            <Text style={row}>Au : <strong>{returnDate}</strong></Text>
            <Text style={row}>Prise en charge : <strong>{pickupLocation}</strong></Text>
            {returnLocation && returnLocation !== pickupLocation && (
              <Text style={row}>Retour : <strong>{returnLocation}</strong></Text>
            )}
            <Text style={row}>Durée : <strong>{rentalDays} jour{rentalDays > 1 ? 's' : ''}</strong></Text>
          </Section>

          <Section style={section}>
            <Text style={sectionTitle}>TARIFICATION</Text>
            <Text style={row}>Véhicule ({rentalDays}j × {fmt(dailyRate)} MAD) : <strong>{fmt(vehicleTotal)} MAD</strong></Text>
            {addonsDetails.map((a, i) => (
              <Text key={i} style={row}>{a.name} : <strong>{fmt(a.total)} MAD</strong></Text>
            ))}
            {deliveryFee > 0 && (
              <Text style={row}>Frais de livraison : <strong>{fmt(deliveryFee)} MAD</strong></Text>
            )}
            {discountAmount > 0 && (
              <Text style={row}>Réduction : <strong>-{fmt(discountAmount)} MAD</strong></Text>
            )}
            <Hr style={divider} />
            <Text style={row}>Caution : <strong>{fmt(depositAmount)} MAD</strong></Text>
...
export const template = {
  ...
  subject: (data: Record<string, any>) => `Confirmation de réservation N° ${data.confirmationId || ''}`,
  ...
  previewData: {
    ...
    returnDate: '18 janvier 2025',
    pickupLocation: 'Aéroport Mohammed V',
    returnLocation: 'Casablanca Centre',
    rentalDays: 3,
    dailyRate: 350,
    vehicleTotal: 1050,
    addonsDetails: [{ name: 'Siège bébé (3j)', total: 150 }],
    deliveryFee: 100,
    discountAmount: 0,
    depositAmount: 3000,
    totalPrice: 1300,
