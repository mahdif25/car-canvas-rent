...
function getMaxPriceFromTiers(tiers: { daily_rate: number }[]): number {
  if (!tiers.length) return 0;
  return Math.max(...tiers.map((t) => Number(t.daily_rate)));
}
...
const WhatsAppPopup = () => {
  ...
  // Poll for autofill value — browsers fill at unpredictable times
  useEffect(() => {
    let attempts = 0;
  ...
  // Detect Chrome autofill via animation event
  const handleAnimationStart = (e: React.AnimationEvent<HTMLInputElement>) => {
    if (e.animationName === "onAutoFillStart" || e.animationName.includes("auto-fill")) {
      setTimeout(() => {
        if (nameInputRef.current?.value) {
          setAutofillName(nameInputRef.current.value);
        }
  ...
  const openWhatsApp = () => {
    ...
    const msg = encodeURIComponent(
      `Bonjour, je souhaite louer la ${selectedVehicle?.name} pour ${days} jour${days! > 1 ? "s" : ""}. D'après vos tarifs, le prix serait de ${rate} MAD/jour. Récupération à ${pickupLocation}. Merci !`
    );
  ...
  return (
    <>
      {/* Hidden autofill form — must be a real form for browsers to autofill */}
      <form autoComplete="on" style={{ position: "fixed", top: -100, left: -100, width: 1, height: 1, overflow: "hidden" }} tabIndex={-1} aria-hidden="true">
        <input
          ref={nameInputRef}
          type="text"
          name="fname"
              ...
              {step === 1 && (
                <div className="space-y-2 mb-3">
                  {availableVehicles.map((v) => {
                    const vehicleTiers = tiers?.filter((t) => t.vehicle_id === v.id) ?? [];
                    const minRate = getStartingPriceFromTiers(vehicleTiers);
                    const maxRate = getMaxPriceFromTiers(vehicleTiers);
                    return (
                      <button
                        key={v.id}
                  ...
                  <BotBubble>
                    ...
                    {getDailyRate() > 0 && (
                      <p className="mt-1 font-semibold">
                        Pour une location de {days} jour{days! > 1 ? "s" : ""}, le tarif est de {getDailyRate()} MAD/jour
                      </p>
                    )}
                  </BotBubble>
